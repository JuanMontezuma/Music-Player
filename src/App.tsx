...
const audioStorage: { [key: number]: string } = {};
...
const MusicPlayer: React.FC = () => {
  ...
  const loadSongs = async () => {
    try {
      ...
      if (data.success) {
        // Recuperar URLs de audio del almacenamiento local
        const songsWithAudio = data.data.map((song: Song) => ({
          ...song,
          audioUrl: audioStorage[song.id] || undefined
        }));
  ...
  const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (data.success) {
        // Crear URL del archivo de audio y guardarla
        const audioUrl = URL.createObjectURL(file);
        audioStorage[data.data.id] = audioUrl;
        ...
        alert(`Canción agregada: ${songNameFromFile}${artistFromFile ? ' - ' + artistFromFile : ''}`);
      }
    } catch (error) {
  ...
  const removeSong = async (songId: number) => {
    try {
      ...
      if (data.success) {
        ...
        if (audioStorage[songId]) {
          URL.revokeObjectURL(audioStorage[songId]);
          delete audioStorage[songId];
        }
  ...
  const togglePlayPause = () => {
    ...
    if (!currentSong.audioUrl) {
      alert('Esta canción no tiene archivo de audio. Por favor súbela de nuevo.');
      return;
    }
                ...
                <>
                  ...
                  {!currentSong.audioUrl && (
                    <p style={{color: '#ef4444', fontSize: '0.875rem'}}>
                      ⚠️ Sin archivo de audio - Sube el archivo de nuevo
                    </p>
                  )}
                ...
                >
                  ...
                  <div className="playlist-actions">
                    <button
                      ...
                      style={{opacity: song.audioUrl ? 1 : 0.5}}
                    >
                      Reproducir
                    </button>
